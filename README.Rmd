...
We install and load the developmental version of MIAMI.
...
We start by showing how to compute the measures, EQMI\*, EQMI, and CSQMI (Principe 2010) between arbitrary random variables (r.v.'s) where the data does not necessarily have a multiplex imaging data structure. We create a matrix named Data_matrix with 5000 samples and 5 columns corresponding to five r.v.'s.
...
Here, we show the mathematical expressions of the three different measures, EQMI*, EQMI and CSQMI, implemented in this package. Further details can be found in the manuscript, "MIAMI: Mutual Information-based Analysis of Multiplex Imaging data". 

$$
\begin{align}
&\text{EQMI}^*(X_{1j}, X_{2j}, \ldots, X_{pj}) = \frac{V_J -2V_C + V_M}{V_J + V_M}\\
&\text{EQMI}(X_{1j}, X_{2j}, \ldots, X_{pj}) = V_J -2V_C + V_M\\
&\text{CSQMI}(X_{1j}, X_{2j}, \ldots, X_{pj}) = \log(V_J) -2\log(V_C) + \log(V_M)\\
&V_J = \int \int \ldots \int f_{12 \ldots pj}(x_{1}, x_{2}, \ldots , x_{p})^2dx_{1}dx_{2} \ldots dx_{p} \\
&V_C = \int \int \ldots \int f_{12 \ldots pj}(x_{1}, x_{2}, \ldots , x_{p})f_{1j}(x_{1})f_{2j}(x_{2}) \ldots f_{pj}(x_{p})dx_{1}dx_{2} \ldots dx_{p}\\
&V_M = \int \int \ldots \int f_{1j}(x_{1})^2f_{2j}(x_{2})^2 \ldots f_{pj}(x_{p})^2 dx_{1}dx_{2} \ldots dx_{p}.
\end{align}
$$
